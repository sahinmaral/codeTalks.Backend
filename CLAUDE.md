...
- **Redis** — `AddInfrastructureService` calls `ConnectionMultiplexer.Connect` **eagerly at registration time**, *before* `ConfigureTestServices` runs, so it can't simply be removed after the fact. The factory sets `Redis:ConnectionString` to `…,abortConnect=false` so that eager connect doesn't throw, then swaps `IConnectionMultiplexer` for an NSubstitute stub.
