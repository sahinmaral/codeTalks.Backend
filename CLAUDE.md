...
The photo endpoints do their multipart binding + FluentValidation + owner-authorization + persistence for real against Postgres; only the Cloudinary calls themselves are faked (upload returns a canned secure URL, delete is asserted via `Received`), so the assertions cover everything except the actual asset transfer. What the suite *cannot* cover is the transport layer described under "Photo uploads" below: `TestServer` is not Kestrel, so neither the data-rate relaxation nor the request-size limit is exercised by these tests.
...
Two endpoints take a multipart `IFormFile`: `PUT /api/users/profile-photo` and `PUT /api/channels/{channelId}/thumbnail-photo`. Content type and size are validated in `ImageFileRules` (`codeTalks.Application/Services/FileStorage/`), which caps a file at `MaxFileSizeInBytes` (5 MB) and returns a readable `BusinessException` message.

Both endpoints also carry two transport-level attributes, because the app-layer rules only run *after* the body has already been read off the wire:

- **`[RequestSizeLimit(ImageFileRules.MaxRequestBodySizeInBytes)]`** — a hard ceiling on the request body, set to the file cap **plus 512 KB of headroom** for multipart framing. The headroom is deliberate: it keeps a file that is only slightly over 5 MB inside the limit so `ImageFileRules` rejects it with its own message, instead of Kestrel cutting it off with a bare 413. `MaxFileSizeInBytes` is `public` solely so this attribute can be expressed in terms of it — the two limits must never drift apart.
- **`[AllowSlowUploads]`** (`codeTalks.Presentation/Filters/AllowSlowUploadsAttribute.cs`) — an `IResourceFilter` that lowers Kestrel's `MinRequestBodyDataRate` from its 240 bytes/s default to 100 bytes/s with a 30-second grace period. The default floor is tuned for JSON, not for multi-megabyte photos arriving from a phone on a cellular link, where a few seconds of stall mid-upload is routine; when it trips, Kestrel answers with a 400 that leaks the internal `MinRequestBodyDataRate` detail. The rate is **lowered, not removed**, so a genuinely dead connection is still cut and nobody can hold a connection open by drip-feeding bytes — and it is only ever paired with the size limit above, so a relaxed rate can never apply to an unbounded body. A resource filter is the right hook because it runs *before* model binding, and binding the `IFormFile` is what actually reads the body.

The filter no-ops when `IHttpMinRequestBodyDataRateFeature` is absent, which is exactly the case under `TestServer` — there is no Kestrel and no data rate there. That is why the integration suite cannot cover either attribute (see the Photos slice notes above).

`ThumbnailPhotoURL` is surfaced on both channel read DTOs (`ChannelByIdDto`, `ChannelsByUserIdItemDto`) so a client can render a channel's thumbnail from either the detail endpoint or the discovery list without a second call.
